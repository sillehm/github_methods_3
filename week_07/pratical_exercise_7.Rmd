...
```{python}
y_noise = y + np.random.normal(loc = 0, scale = 5, size = 60)
```
...
plt.plot(x, y, "m-")
plt.plot(x, y_noise, "ko")
plt.legend(["Predicted line", "True function", "Observations"])
plt.show()
...
X_quadratic is our design matrix 
...
y_fifth_hat = regressor.predict(X_fifth) 
```
...
plt.plot(x, y_hat, "c--")
plt.plot(x, y, "m-")
plt.plot(x, y_quadratic_hat, "y-.")
plt.plot(x, y_fifth_hat, 'b:')
plt.legend(["Generated datapoints", "Predicted line", "True function", "Quadratic function", "Fifth order fit"])
plt.show()
```
...
# Creating 100 lists inside a list
y_noise_samples = [y + np.random.normal(loc = 0, scale = 5, size = 60) for i in range(100)]
...
predicted_linear_hats = []

for sample in y_noise_samples:
  regressor.fit(x, sample)
  predicted_linear_hats.append(regressor.predict(x))

#quadratic fit
regressor = LinearRegression()

predicted_quadratic_hats = []

for sample in y_noise_samples:
  regressor.fit(x, sample)
  predicted_quadratic_hats.append(regressor.predict(x))

# fifth order fit
regressor = LinearRegression()

predicted_fifth_hats = []

for sample in y_noise_samples:
  regressor.fit(x, sample)
  predicted_fifth_hats.append(regressor.predict(x))
...
```
    ...
    Understanding iii
```{python}
for y_hat_sample in predicted_linear_hats:
    lines = plt.plot(x, y_hat_sample, 'r-')
    lines = plt.xlabel('')
    lines = plt.ylabel('')

for y_hat_sample in predicted_quadratic_hats:
    lines = plt.plot(x, y_hat_sample, 'b-')
    lines = plt.xlabel('')
    lines = plt.ylabel('')
...
plt.plot(x_3, y_9, "y*", 15)
plt.legend(["Linear", "Quadratic", "True value for x0=3"])
plt.show()
```
